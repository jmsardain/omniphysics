...
void mySidewaysText(Double_t x,Double_t y,Color_t color, const char *text) {
...
void myLineText(Double_t x,Double_t y,Int_t color,Int_t mstyle, const char *text,Float_t msize)
{
  Double_t tsize=0.06;
  TLine *marker = new TLine(x-(0.7*tsize),y, x-(0.1*tsize), y);
  marker->SetLineColor(color);  
	marker->SetNDC();
  marker->SetLineStyle(mstyle);
  marker->SetLineWidth(msize);
  marker->Draw();
...
void myErrorText(Double_t x,Double_t y,Int_t color,Int_t mstyle, const char *text,Float_t msize, TH1D* hist, TPad* c1)
{
  //Double_t tsize=0.06;
  Double_t tsize=0.1;
  //Double_t tsize=10;
  //TLine *marker = new TLine(x-(0.7*tsize),y, x-(0.1*tsize), y);
  TLine *marker = new TLine(x-(0.7*tsize),y, x-(0.1*tsize), y);
  marker->SetLineColorAlpha(color, 0.4);
  marker->SetNDC();
  marker->SetLineStyle(4050);
  marker->SetLineWidth(12);
  //marker->Draw();
  Double_t xf[4],yf[4];
  xf[0] = x - 0.7*tsize;
  yf[0] = y - tsize;
  ...
  yf[3] = yf[2];
  for (Int_t i=0;i<4;i++) {
    xf[i] = c1->GetX1() + xf[i]*(c1->GetX2()-c1->GetX1());
    yf[i] = c1->GetY1() + yf[i]*(c1->GetY2()-c1->GetY1());
  }
  //gPad->PaintFillAreaHatches(4,xf,yf, 4050);
  //gPad->PaintFillArea(4,xf,yf);
  //c1->PaintFillArea(4,xf,yf);
  
  std::cout << __LINE__ << std::endl;
  //c1->PaintFillAreaHatches(4,xf,yf, 3000);
  c1->SetFillColor(kBlack);
  c1->PaintFillArea(4,xf,yf);
  std::cout << __LINE__ << std::endl;

  /*
   TBox *box2 = new TBox(x-(0.7*tsize),y-0.3, x-(0.1*tsize), y+0.03);
   box2->SetFillStyle(mstyle);
   box2->SetNDC();
   box2->SetFillColorAlpha(color, 1.0);
   box2->SetLineWidth(0);
   box2->Draw();
*/
